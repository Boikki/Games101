                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    tExit = std::min(tMax.x, std::max(tMax.y, tMax.z));

    // 注意=问题
    // 因为这次的模型里墙壁和天花板地板这些都是没有厚度的
    // 所以相等的情况必须被算作相交，不然就可能出现右边全黑的情况
    //
    // 其实理论上浮点数的相等应该是相差小于一个较小的数
    // 不过这里因为 tEnter 和 tExit 不是计算出来的
    // 而是 tMin 和 tMax 中的最大最小值
    // 所以两个相等的时候是完全相等的
    // 不会有浮点数计算后导致的误差
    // 所以这里直接把等号加上就好了
    // 不需要相减小于 Epsilon
